...
public class ModifierCollection {
  ...
  private bool ValidateType(string type) {
    ...
    Debug.LogError("Error: " + type + " is not a valid modifier in this collection");
    return false;
  }
  ...
  public int Compute(Character target, string type, int baseValue) {
    ...
    foreach(Modifier mod in Mods[type]) {
      ...
      // Debugging info
      //Debug.LogError($"Evaluating modifier {mod.Name}: {mod.Type} => {(string)mod.Expr}");
...
public class Modifier {
  ...
  public bool IsActive(Character target) {
    ...
    if(!result.Success) {
      //ERORR
      Debug.LogError($"Modifier \"{Name}\" Condition Error:\n{result.Message}");
      return false;
    }
  ...
  public ExpressionResult Evaluate(Character target) {
    ...
    if(!result.Success) {
      //ERORR
      Debug.LogError($"Modifier \"{Name}\" Expression Error:\n{result.Message}");
      return result;
    }
    return result;
  }
